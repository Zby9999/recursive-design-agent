...
加入第一个真实 adapter path，用于 headless external agents。Bridge 应能调用配置好的 CLI command，传入 task payload，收集 structured output，并复用 mocked adapter 已经使用的 validation、event 和 task lifecycle。
...
- [ ] Bridge 支持配置 headless CLI AgentAdapter，且不需要修改 Web App code。
...
- [ ] Bridge 不嵌入或实现 Figma MCP；任何 Figma MCP dependency 都留在 external agent environment 内。
